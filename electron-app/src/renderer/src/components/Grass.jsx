...
const Grass = ({ width = 600, height = 400, parentId = 'sketch-container', humidity }) => {
  const visualisation = ({ width, height, humidity }) => {
    let grass
    const s = (p5) => {
      p5.preload = () => {
        // if (!img) {
      ...
      function flower(x, y, humidity) {
        //drawFlower();
        const adjustment = 1.5
        const size = humidity * adjustment
        p5.push()
        p5.fill(250, 250 - 1.5 * humidity, 100 + humidity, 240)
        p5.translate(x, y)
        p5.noStroke()
        for (var r1 = 0; r1 < 10; r1++) {
          p5.ellipse(0, 10 + size / 20, 10 + size / 40, 20 + size / 20)
          p5.rotate(p5.PI / 5)
        }
      ...
      function yard(p5) {
        ...
        for (let x = 0; x < width; x += width / this.population) {
          ...
          this.grass.push({ x, showFlower: x % 4 === 0 })
          ...
          this.size.push(p5.random(humidity - 10, humidity + 10))
        ...
        this.update = function () {
          ...
          for (let i = 0; i < this.index; i++) {
            ...
            p5.translate(this.grass[i].x, height * 0.65)
            ...
            this.blade(len, i, this.grass[i].showFlower)
        ...
        this.blade = function (len, ind, showFlower) {
          ...
          if (ind / 2 === p5.int(ind / 2)) {
            ...
            p5.stroke(p5.map(humidity, 100, 0, 0, 255), 255 - len * 1.5, len * 1.5, 255)

            /// Using the rotational values, we can
            /// get our random noise and map the return values
            /// to fractions of PI.
            this.rot = p5.map(
              p5.noise(this.roff[ind]),
              0,
              1,
              -p5.QUARTER_PI * 0.1,
              p5.QUARTER_PI * 0.1
          ...
          if (ind / 2 != p5.int(ind / 2)) {
            ...
            p5.stroke(p5.map(humidity, 100, 0, 0, 255), 255 - len * 1.2, len * 1.05, 255)
            // stroke(255 - (len * 2.5), len * 2.5, 10, 255);
            this.rot = p5.map(
              -p5.sin(this.roff[ind]),
              -1,
              1,
              -p5.QUARTER_PI * 0.01,
              p5.QUARTER_PI * 0.01
          ...
          /// makes a new segment.
          p5.strokeWeight(len * 2 * p5.random(0.07, 0.11))
          ...
          if (len > 20) {
            this.blade(len * this.seg[ind], ind, showFlower)
          } else {
            if (showFlower) {
              p5.circle(0, 0, 10)
              flower(0, 0, humidity * 1.2)
            }
          }
        }
      }
  ...
  useEffect(() => {
    const cappedHumidity = humidity < 10 ? 10 : humidity > 95 ? 95 : humidity
    const { cleanup } = visualisation({
      width,
      height,
      humidity: cappedHumidity
    })
